...
exports.getEmployeesListToSearchBy = exports.uploadNewActionFiles = exports.createNewAction = exports.getToWhomCanIRespondWithNewAction = exports.getActionsByWorkflowId = exports.getToWhomCanISendNewWorkflow = exports.uploadNewWorkflowFiles = exports.createNewWorkflow = void 0;
...
const appendAdditionalInfoToEmpPosList = (availableConsignees) => __awaiter(void 0, void 0, void 0, function* () {
    //Queries to Find the `position`, the `jobTitle`, and the `fullName` of each recipient:
    for (let i = 0; i < availableConsignees.length; i++) {
        const posRecord = yield models.Position.findOne({
            where: { id: availableConsignees[i].positionId },
            attributes: ["description"]
        });
        const jobTitleRecord = yield models.JobTitle.findByPk(availableConsignees[i].jobTitleId);
        const empRecord = yield models.Employee.findByPk(availableConsignees[i].employeeId);
...
const getWhoCanIMessageAccordingToTheHierarchy = (req, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const senderEmployee_PositionRecord = yield models.Employee_Position.findOne({
        ...
        if (myClassification === "responsible" /* RESPONSIBLE */) {
            // -------- Case 1: Direct Responsible for a responsible employee --------
            let myParentPositionRecord = yield models.Position.findOne({
                where: { id: myPositionId },
                attributes: ["parentId"]
            });
            const myParentPositionId = myParentPositionRecord.parentId;
            myDirectResponsible = yield models.Employee_Position.findOne({
                where: { positionId: myParentPositionId, classification: "responsible" /* RESPONSIBLE */ },
            ...
            let listOfMySiblingsPositionsIDs = yield models.Position.findAll({
                where: { parentId: myParentPositionId, id: { [sequelize_2.Op.ne]: myPositionId } },
                attributes: ["id"],
            });
            listOfMySiblingsPositionsIDs = listOfMySiblingsPositionsIDs.map((obj) => obj.id);
            myColleagues = yield models.Employee_Position.findAll({
                where: {
                    classification: "responsible" /* RESPONSIBLE */,
                    positionId: { [sequelize_2.Op.in]: listOfMySiblingsPositionsIDs }
                },
                attributes: ["id", "positionId", "jobTitleId", "employeeId"],
                raw: true,
            });
            // -------- Case 3: Subordinates (Staff) for a responsible employee --------
            myChildrenPositions = yield models.Position.findAll({
                where: { parentId: myPositionId },
            });
            listOfChildrenPositionsIDs = myChildrenPositions.map((positionObj) => positionObj.id);
            myStaff = yield models.Employee_Position.findAll({
                where: {
                    [sequelize_2.Op.or]: [
                        {
                            ...
                            [sequelize_2.Op.and]: [
                                ...
                                { classification: { [sequelize_2.Op.ne]: "responsible" /* RESPONSIBLE */ }, }
                            ],
                        },
                        {
                            // 2. My staff from my children positions (if exist)
                            positionId: { [sequelize_2.Op.in]: listOfChildrenPositionsIDs },
            ...
            myColleagues = yield models.Employee_Position.findAll({
                where: {
                    positionId: myPositionId,
                    id: { [sequelize_2.Op.ne]: myEmployee_PositionRecordId },
                    classification: { [sequelize_2.Op.ne]: "responsible" /* RESPONSIBLE */ }
                },
                attributes: ["id", "positionId", "jobTitleId", "employeeId"],
                raw: true,
        ...
        let availableConsignees = [Object.assign(Object.assign({}, myDirectResponsible), { consigneeType: Workflow_Participants_1.ConsigneeTypes.DIRECT_RESPONSIBLE })];
        myColleagues.forEach((colleagueObj) => {
            ...
            availableConsignees.push(Object.assign(Object.assign({}, colleagueObj), { consigneeType: Workflow_Participants_1.ConsigneeTypes.COLLEAGUE }));
        });
        myStaff === null || myStaff === void 0 ? void 0 : myStaff.forEach((staffObj) => {
            // const onlyTheRecordObj = staffObj.dataValues; // since raw: true, no need to say: staffObj.dataValues
            availableConsignees.push(Object.assign(Object.assign({}, staffObj), { consigneeType: Workflow_Participants_1.ConsigneeTypes.STAFF }));
        });
        // ----------------------------------------------------------------------------------
        availableConsignees = yield appendAdditionalInfoToEmpPosList(availableConsignees);
        return availableConsignees;
    }
    catch (error) {
        next(error);
...
const appendMyCustomConsigneesGroups = (req, availableConsignees) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const myCustomGroups = yield models_1.default.sequelize.query(getMyCustomGroups, {
        logging: console.log,
    ...
    for (let i = 0; i < myCustomGroups.length; i++) {
        ...
        customGroup.consigneeType = Workflow_Participants_1.ConsigneeTypes.CUSTOM_GROUP;
        const getMembers = `SELECT memberId FROM consignees_groups_members as CGM WHERE CGM.groupId = ${customGroup.id}`;
        let groupMembers = yield models_1.default.sequelize.query(getMembers, {
            logging: console.log,
            type: sequelize_1.QueryTypes.SELECT
        });
        // -------- Convert the array of objects to array of numbers (IDs) only: ----
        groupMembers = groupMembers.map((member) => member.memberId);
        customGroup.groupMembers = groupMembers;
        // --- Prepend the object to the list: `availableConsignees`:
        availableConsignees === null || availableConsignees === void 0 ? void 0 : availableConsignees.unshift(customGroup);
    }
    return availableConsignees;
});
// ---------- End Internal Function contains Common Code ----------
//  ------------- `Create New Workflow` page -------------
const createNewWorkflow = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const newWorkflow = yield models.Workflow.create({
            subject, workflowType, priority
        });
        // -------- Create New `Action` Record --------
        const newAction = yield models.Action.create({
            content: richTextContent,
            workflowId: newWorkflow.id
        });
        // -------- Create New `Workflow_Participant` Record for the SENDER --------
        const newSender = yield models.Workflow_Participant.create({
            workflowId: newWorkflow.id,
            actionId: newAction.id,
            // empPositionId: req.user.employeePositionId,
            empPositionId: req.header("employeeCurrentPositionId"),
            actionType: Workflow_Participants_1.ActionTypes.SENDER,
            isSeen: true,
        });
        // --- Now for each recipient in the list, create new `Workflow_Participant` Record --
        for (let i = 0; i < recipients.length; i++) {
            const newRecipient = yield models.Workflow_Participant.create({
                workflowId: newWorkflow.id,
                actionId: newAction.id,
                empPositionId: recipients[i].id,
                actionType: Workflow_Participants_1.ActionTypes.RECIPIENT
            });
        }
        // --- Now for each CC in the list, create new `Workflow_Participant` Record --
        for (let i = 0; i < cc.length; i++) {
            const newCC = yield models.Workflow_Participant.create({
                workflowId: newWorkflow.id,
                actionId: newAction.id,
                empPositionId: cc[i].id,
                actionType: Workflow_Participants_1.ActionTypes.CC
            });
        }
        return res.json({
...
const uploadNewWorkflowFiles = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        if (req.files === null) {
            return res.json({
        ...
        let files = (_a = req.files) === null || _a === void 0 ? void 0 : _a.workflowFiles; //now files is array of objects, each object stores a single file info.
        ...
        const maxNumberOfAllowedFiles = attachment_1.default.MAX_NUMBER_OF_FILES;
        let feedbackList = [];
        ...
        if (files.length > maxNumberOfAllowedFiles) {
            ...
            feedbackList.push(`The max number of allowable files is ${maxNumberOfAllowedFiles}, only first ${maxNumberOfAllowedFiles} files have been accepted`);
        ...
        files = files.filter((file) => {
            if (filesBlackList.includes(file.mimetype)) {
                ...
                feedbackList.push(`REJECTED file: ${file.name} because of its mimetype: ${file.mimetype}`);
                return;
            }
            const extension = path_1.default.extname(file.name); //extension name with the dot (.) ex: .png
            if (extension == ".exe") {
                // reject the file
                console.log(`REJECTED a file whose extension = ${file.mimetype}`);
                feedbackList.push(`REJECTED file: ${file.name} because of its extension/mimetype: ${file.mimetype}`);
                return;
            }
            if (file.truncated) {
                // don't reject the file, just tell the user that it has been truncated.
                feedbackList.push(`Truncated file: ${file.name} due to its large size`);
            }
        ...
        const myPath = path_1.default.join(__dirname, "../../", "public", "actions_attachments");
        console.log("myPath: ", myPath);
        // Iterate over the files list and prepare to move each file to its destination
        let promises = files.map((file) => {
            const extension = path_1.default.extname(file.name); //extension name with the dot (.) ex: .png
            const uniqueName = `${workflowId}_${actionId}_${new Date().getTime()}${extension}`;
        ...
        yield Promise.all(promises); //Now execute the array of promises 
        // -------------- Done Uploading Files 😇✅ -------------------------
        // ---- Storing Files info in the database table `Actions_Attachments` -----
        for (let i = 0; i < files.length; i++) {
            let file = files[i];
            const newAttachment = yield models.Action_Attachment.create({
...
exports.uploadNewWorkflowFiles = uploadNewWorkflowFiles;
const getToWhomCanISendNewWorkflow = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Remember that the middleware function "validateAccessToken"
        // has added a `req.user` entry to our request.
        const { employeePositionId } = req.user; //from the middleware function 
        let availableConsignees = yield getWhoCanIMessageAccordingToTheHierarchy(req, next);
        yield appendMyCustomConsigneesGroups(req, availableConsignees);
...
exports.getToWhomCanISendNewWorkflow = getToWhomCanISendNewWorkflow;
//  ------------- `Actions of a Workflow` page -------------
const getActionsByWorkflowId = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    // Remember that the middleware function "validateAccessToken"
    // has added a `req.user` entry to our request.
    // const employeePositionId = req.user.employeePositionId 
    const employeePositionId = req.header("employeeCurrentPositionId");
    const workflowId = Number(req.query.workflowId);
    const cameFromActionId = Number(req.query.cameFromActionId);
    ...
    try {
        ...
        const workflowRecord = yield models.Workflow.findByPk(workflowId);
        ...
        const recordsIAMInvolvedIn = yield models.Workflow_Participant.findOne({
        ...
        yield models.Workflow_Participant.update({ isSeen: true }, {
            where: {
                workflowId,
        ...
        const actionsIDsWhereIWasMentioned = yield models.Workflow_Participant.findAll({
            where: { workflowId: workflowId, empPositionId: employeePositionId },
            attributes: ["actionId"]
        });
        let str = "(";
        actionsIDsWhereIWasMentioned.forEach((obj) => {
            str += obj.actionId + ", ";
        });
    ...
    WHERE A.workflowId = ${workflowId}`;
        const allParticipants = yield models_1.default.sequelize.query(getAllWorkflowParticipants, {
            type: sequelize_1.QueryTypes.SELECT,
            logging: console.log,
        });
        ...
        for (let i = 0; i < allParticipants.length; i++) {
            ...
            const employeePositionRecord = yield models.Employee_Position.findByPk(participant.empPositionId, {
            ...
            const employeeRecord = yield models.Employee.findOne({
            ...
            const jobTitleRecord = yield models.JobTitle.findByPk(employeePositionRecord.jobTitleId);
            participant.jobTitle = jobTitleRecord.title;
            // --- Fetch the participant's position:
            const positionRecord = yield models.Position.findByPk(employeePositionRecord.positionId);
            participant.position = positionRecord === null || positionRecord === void 0 ? void 0 : positionRecord.description;
        }
        ...
        const actions = allParticipants.filter((action) => action.actionType === "SENDER");
        ...
        let allActionsAttachments = []; //here store all attachments regarding this workflow
        for (let i = 0; i < actions.length; i++) {
            let action = actions[i];
            let attachments = [];
            attachments = yield models.Action_Attachment.findAll({
                where: { actionId: action.actionId }
            });
            // `attachments` is an array of `actions_attachments` records
            attachments.map((attObj) => allActionsAttachments.push(attObj));
...
exports.getActionsByWorkflowId = getActionsByWorkflowId;
const getToWhomCanIRespondWithNewAction = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    // Remember that the middleware function "validateAccessToken"
    // has added a `req.user` entry to our request.
    try {
        // const employeePositionId = req.header("employeeCurrentPositionId");
        ...
        let availableConsignees = yield getWhoCanIMessageAccordingToTheHierarchy(req, next);
        const availableConsigneesEmpPosIDs = availableConsignees === null || availableConsignees === void 0 ? void 0 : availableConsignees.map((consignee) => consignee.id);
        ...
        const actionsInWhichIWasMentioned = yield models.Workflow_Participant.findAll({
            where: {
                workflowId,
                empPositionId: employeePositionId,
                actionType: { [sequelize_2.Op.ne]: Workflow_Participants_1.ActionTypes.SENDER }
            },
            attributes: ["actionId"]
        });
        const listOfActionsIDsInWhichIWasMentioned = actionsInWhichIWasMentioned.map((action) => action.actionId);
        const participantsMentionedMeInThisWorkflow = yield models.Workflow_Participant.findAll({
            where: {
                actionType: Workflow_Participants_1.ActionTypes.SENDER,
                actionId: {
                    [sequelize_2.Op.in]: listOfActionsIDsInWhichIWasMentioned,
                }
            }
        });
        const listOfParticipantsEmpPositionIDsWhoMentionedMe = participantsMentionedMeInThisWorkflow.map((pa) => pa.empPositionId);
        ...
        */
        let myNewContactsBecauseTheyMentionedMe = yield models.Employee_Position.findAll({
            where: {
                id: {
                    [sequelize_2.Op.in]: listOfParticipantsEmpPositionIDsWhoMentionedMe,
                    [sequelize_2.Op.notIn]: availableConsigneesEmpPosIDs,
                }
            },
            attributes: ["id", "positionId", "jobTitleId", "employeeId"],
            raw: true,
        });
        myNewContactsBecauseTheyMentionedMe = yield appendAdditionalInfoToEmpPosList(myNewContactsBecauseTheyMentionedMe);
        myNewContactsBecauseTheyMentionedMe === null || myNewContactsBecauseTheyMentionedMe === void 0 ? void 0 : myNewContactsBecauseTheyMentionedMe.forEach((empPosObj) => {
            availableConsignees === null || availableConsignees === void 0 ? void 0 : availableConsignees.unshift(Object.assign(Object.assign({}, empPosObj), { consigneeType: Workflow_Participants_1.ConsigneeTypes.WORKFLOW_PARTICIPANT }));
        });
        // ---- 😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻😻----
...
exports.getToWhomCanIRespondWithNewAction = getToWhomCanIRespondWithNewAction;
const createNewAction = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const workflowRecord = yield models.Workflow.findByPk(workflowId);
        yield models.Workflow.update({ "subject": workflowRecord.subject }, {
            where: { id: workflowId }
        });
...
const uploadNewActionFiles = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let files = (_b = req.files) === null || _b === void 0 ? void 0 : _b.actionFiles; //now files is array of objects, each object stores a single file info.
...
exports.uploadNewActionFiles = uploadNewActionFiles;
//  ------------- `Search Box` page -------------
const getEmployeesListToSearchBy = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // const { employeePositionId } = req.user; //from the middleware function 
        const employeePositionId = req.header("employeeCurrentPositionId");
        ...
        const peopleList = yield models_1.default.sequelize.query(theMiracle, {
            type: sequelize_1.QueryTypes.SELECT,
            logging: console.log,
        });
        return res.json({
...
exports.getEmployeesListToSearchBy = getEmployeesListToSearchBy;
